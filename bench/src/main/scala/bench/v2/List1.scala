...
sealed abstract class List[+A]
  ...
  final override def toList: scala.collection.immutable.List[A] = ???
...
final case class :: [+A](override val head: A, private[v2] var next: List[A @uncheckedVariance]) // sound because `next` is used only locally
  extends List[A] {
  releaseFence()
...
object List extends StrictOptimizedSeqFactory[List] {
  ...
  def newBuilder[A]: mutable.Builder[A, List[A]] = new ListBuffer()
